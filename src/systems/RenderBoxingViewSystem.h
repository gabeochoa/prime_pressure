...
create_order_display_info(int order_number, const Order *order_ptr,
  ...
  if (is_ready) {
    ...
    info.status_text = " - Waiting for items (" + std::to_string(ready_count) +
...
static int
count_unplaced_items(const std::vector<afterhours::EntityID> &boxing_items) {
  int count = 0;
...
static void render_box(float center_x_pct, float center_y_pct, int screen_width,
  ...
  raylib::Vector2 key_pos{box_x + box_width * 0.5f, box_y + box_height * 0.1f};

  if (boxing_progress.state == BoxingState::PutItems) {
    key_pos.x -= 10.0f;
    key_pos.y = box_y + box_height * 0.35f;
    draw_keycap(key_pos, "P", instruction_font_size, key_bg, key_fg, uiFont);
  } else if (boxing_progress.state == BoxingState::Fold) {
    key_pos.x -= 10.0f;
    draw_keycap(key_pos, "F", instruction_font_size, key_bg, key_fg, uiFont);
  } else if (boxing_progress.state == BoxingState::Tape) {
    key_pos.x -= 10.0f;
    draw_keycap(key_pos, "T", instruction_font_size, key_bg, key_fg, uiFont);
...
struct RenderBoxingViewSystem : BoxingViewRenderSystem {
  void once(float) const override {
    ...
    raylib::DrawRectangleGradientV(
        static_cast<int>(layout.boxing.x), static_cast<int>(layout.boxing.y),
        static_cast<int>(layout.boxing.w), static_cast<int>(layout.boxing.h),
        raylib::Color{180, 168, 150, 235}, raylib::Color{128, 116, 98, 235});

    draw_view_box(layout.boxing, active_view.current_view, ViewState::Boxing);
    ...
    render_items_list(left_x, content_start_y, boxing_progress, screen_width,
                      screen_height);
