...
private:
  ...
  LayoutData
  setup_layout([[maybe_unused]] const ActiveView &active_view) const {
    LayoutData data;
    ...
    data.cols =
        std::max(1, std::min(3, static_cast<int>(data.content_w / card_min_w)));
    ...
    for (const auto &[item_type, count] : item_counts) {
      ...
      char icon_char =
          (key_it != ITEM_KEY_MAP.end())
              ? static_cast<char>(
                    std::toupper(static_cast<unsigned char>(key_it->second)))
              : (name.empty() ? '?'
                              : static_cast<char>(std::toupper(
                                    static_cast<unsigned char>(name[0]))));
      auto ready_it = ready_counts.find(item_type);
      int ready_count = ready_it != ready_counts.end() ? ready_it->second : 0;
      ...
      raylib::Vector2 icon_text_size =
          raylib::MeasureTextEx(raylib::GetFontDefault(), icon_text.c_str(),
      ...
      raylib::DrawTextEx(raylib::GetFontDefault(), icon_text.c_str(),
                         ...
                         static_cast<float>(icon_font), 1.0f, raylib::WHITE);
  ...
  void draw_timeline(const raylib::Rectangle &card_rect, float scaled_h,
                     float scaled_w, TimelineStageState stage_value,
                     bool flash_toggle, const raylib::Color &sub_text,
                     const raylib::Color &accent_active,
                     [[maybe_unused]] const LayoutData &data) const {
    ...
    auto get_timeline_atlas = []() -> TimelineAtlas & {
      ...
      if (!atlas.loaded) {
        ...
        atlas.conveyor = raylib::Rectangle{640.0f, 1472.0f, 48.0f, 48.0f};
    ...
    auto stage_color = [&](TimelineStageState s, bool flashing) {
      TimelinePhase phase = timeline_phase(s);
      switch (phase) {
      case TimelinePhase::Done:
        return raylib::Color{91, 159, 59, 255};
      case TimelinePhase::Active:
        if (flashing && flash_toggle) {
          return raylib::Color{255, 200, 120, 255};
        }
        return accent_active;
      case TimelinePhase::Pending:
      default:
        return sub_text;
      }
    };

    auto stage_for = [](int idx, TimelinePhase phase) {
      switch (idx) {
      case 0:
        return phase == TimelinePhase::Active
                   ? TimelineStageState::ConveyorActive
                   : (phase == TimelinePhase::Done
                          ? TimelineStageState::ConveyorDone
                          : TimelineStageState::ConveyorPending);
      case 1:
        return phase == TimelinePhase::Active
                   ? TimelineStageState::BoxingActive
                   : (phase == TimelinePhase::Done
                          ? TimelineStageState::BoxingDone
                          : TimelineStageState::BoxingPending);
      case 2:
        return phase == TimelinePhase::Active
                   ? TimelineStageState::ReadyActive
                   : (phase == TimelinePhase::Done
                          ? TimelineStageState::ReadyDone
                          : TimelineStageState::ReadyPending);
      default:
        return phase == TimelinePhase::Done ? TimelineStageState::ShipDone
                                            : TimelineStageState::ShipPending;
      }
    };

    auto current_index = [](TimelineStageState s) {
      switch (s) {
    ...
    int current_stage_index = current_index(stage_value);

    auto stage_for_index = [&](int index) {
      if (index < current_stage_index) {
        return stage_for(index, TimelinePhase::Done);
      }
      if (index == current_stage_index) {
        return stage_value;
      }
      return stage_for(index, TimelinePhase::Pending);
    };

    std::array<TimelineStageState, 4> states{
        stage_for_index(0), stage_for_index(1), stage_for_index(2),
        stage_for_index(3)};
    ...
    draw_timeline_lines(timeline_left, timeline_right, timeline_y, segment_w,
                        stages, states, stage_color, radius);
    draw_timeline_icons(timeline_left, timeline_right, timeline_y, segment_w,
                        stages, states, sources, timeline_atlas, stage_color,
                        stage_value);
  }

  TimelineStageState stage_for(int idx, TimelinePhase phase) const {
    switch (idx) {
    case 0:
      return phase == TimelinePhase::Active
                 ? TimelineStageState::ConveyorActive
  ...
  void draw_timeline_lines(
      ...
      const std::array<TimelineStageState, 4> &states,
      const std::function<raylib::Color(TimelineStageState, bool)> &stage_color,
      float radius) const {
    for (int i = 0; i < stages - 1; ++i) {
      float x1 = timeline_left + segment_w * static_cast<float>(i);
      float x2 = timeline_left + segment_w * static_cast<float>(i + 1);
      TimelinePhase p1 = timeline_phase(states[i]);
      TimelinePhase p2 = timeline_phase(states[i + 1]);
      TimelinePhase line_phase =
          (p1 == TimelinePhase::Active || p2 == TimelinePhase::Active)
              ? TimelinePhase::Active
          : (p1 == TimelinePhase::Done && p2 == TimelinePhase::Done)
              ? TimelinePhase::Done
              : TimelinePhase::Pending;
      TimelineStageState line_state = stage_for(std::min(i, i + 1), line_phase);
      raylib::DrawLineEx(raylib::Vector2{x1 + radius, timeline_y},
                         raylib::Vector2{x2 - radius, timeline_y}, 3.0f,
                         stage_color(line_state, timeline_phase(line_state) ==
                                                     TimelinePhase::Active));
    }
  }
  ...
  void draw_timeline_icons(
      ...
      const std::function<raylib::Color(TimelineStageState, bool)> &stage_color,
      [[maybe_unused]] TimelineStageState stage_value) const {
    for (int i = 0; i < stages; ++i) {
      float cx = timeline_left + segment_w * static_cast<float>(i);
      TimelineStageState st = states[i];
      float icon_size_px = 28.0f;
      raylib::Rectangle dst{cx - icon_size_px * 0.5f,
                            timeline_y - icon_size_px * 0.5f, icon_size_px,
                            icon_size_px};
      raylib::DrawTexturePro(
          timeline_atlas.texture, sources[i], dst, raylib::Vector2{0.0f, 0.0f},
          0.0f,
          stage_color(st, st == TimelineStageState::ConveyorActiveFlash ||
                              st == TimelineStageState::BoxingActive ||
                              st == TimelineStageState::ReadyActive ||
                              (st >= TimelineStageState::ReadyStamp0 &&
                               st <= TimelineStageState::ReadyStamp3)));
    }
  }
  ...
  void draw_pack_banner(const raylib::Rectangle &card_rect, float scaled_w,
    ...
    if (ready_to_pack && !is_shipped) {
      ...
      raylib::Vector2 pack_pos{card_rect.x + (scaled_w - pack_size.x) * 0.5f,
                               card_rect.y + card_rect.height - 26.0f -
                                   pack_size.y - 6.0f};
    ...
    if (is_shipped) {
      ...
      for (size_t i = 0; i < stamp_words.size(); ++i) {
        raylib::Color active_color =
            (ready_stamp >= static_cast<int>(i + 1)) ? green_stamp : gray_hint;
        raylib::Vector2 key_size = raylib::MeasureTextEx(
            raylib::GetFontDefault(), stamp_keys[i].c_str(),
            static_cast<float>(stamp_font), 1.0f);
        float key_w = key_size.x + padding * 2.0f;
        draw_keycap(raylib::Vector2{cursor_x, stamp_y}, stamp_keys[i],
                    stamp_font, raylib::Color{24, 24, 24, 230}, active_color,
                    raylib::GetFontDefault(), padding);
        ...
        raylib::DrawTextEx(raylib::GetFontDefault(), stamp_words[i].c_str(),
                           ...
                           static_cast<float>(stamp_font), 1.0f, active_color);
  ...
  void render_single_card(afterhours::EntityID order_id, int order_index,
    ...
    for (const Order &order : afterhours::EntityQuery()
      ...
      float card_h = std::max(is_selected ? data.card_h_base + 24.0f
                              ...
                              100.0f);
      float card_x = data.content_x + data.pad_px +
                     static_cast<float>(col) * (data.card_w + data.pad_px);
      float card_y =
          data.content_y + static_cast<float>(row) * (card_h + data.pad_px);
      ...
      bool is_shipped = order.is_shipped();
      int ready_stamp = order.get_ready_stamp_progress();
      const TimelineState &timeline = order.timeline;
      bool ready_to_pack = order.is_ready_to_pack();
      bool should_flash_conveyor = order.should_flash_conveyor();
      TimelineStageState stage_value = timeline.state;
      ...
      raylib::Color title_shadow =
          is_active
              ? accent_dark
              : (is_selected ? accent_dark : raylib::Color{120, 130, 150, 255});
      ...
      raylib::Rectangle shadow_rect{card_x - offset_x + shadow_offset,
                                    card_y - offset_y + shadow_offset, scaled_w,
                                    scaled_h};
      ...
      float title_h = 28.0f;
      raylib::Rectangle title_rect{card_rect.x, card_rect.y, scaled_w, title_h};
      ...
      raylib::DrawRectangleRoundedLines(
          raylib::Rectangle{card_rect.x + 1.0f, card_rect.y + 1.0f,
                            card_rect.width - 2.0f, card_rect.height - 2.0f},
          0.02f, 4, edge_dark);

      bool flash_toggle = (static_cast<int>(raylib::GetTime() * 2.0f) % 2) == 0;

      int title_font = data.body_font_size;
      std::string base_title = "Order " + std::to_string(order_index + 1) +
      ...
      if (!action_text.empty() && !flash_toggle) {
        title =
            "Order " + std::to_string(order_index + 1) + " - " + action_text;
      }
      raylib::DrawTextEx(
      ...
      draw_timeline(card_rect, scaled_h, scaled_w, stage_value, flash_toggle,
                    sub_text, accent_active, data);

      draw_pack_banner(card_rect, scaled_w, ready_to_pack, is_shipped, data);
      draw_stamp_prompt(card_rect, scaled_h, scaled_w, is_shipped, ready_stamp,
                        data);
      break;
    }
  }
