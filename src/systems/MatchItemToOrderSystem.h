...
struct MatchItemToOrderSystem
    : afterhours::System<Order, OrderWorkflow, OrderRequiredCounts, OrderRequestedCounts,
  ...
  void for_each_with(afterhours::Entity &order_entity, Order &order, OrderWorkflow &workflow,
                     OrderRequiredCounts &required_counts, OrderRequestedCounts &requested_counts, float) override {
    ...
    for (const auto &[item_type, required_count] : required_counts.counts) {
      ...
      order.selected_items.push_back(item_type); // Keep for compatibility
      mark_conveyor_item_as_moving(order_entity.id, item_type);

      // Check if all items are now requested
      bool all_requested = true;
      for (const auto &[check_type, check_required] : required_counts.counts) {
        auto check_requested_it = requested_counts.counts.find(check_type);
        int check_requested = (check_requested_it != requested_counts.counts.end()) ? check_requested_it->second : 0;
      ...
      if (all_requested) {
        ...
        log_info("Order {} all items requested, advancing to AllRequested", static_cast<unsigned long long>(order_entity.id));
      }
    ...
    workflow.time_in_state = 0.0f;
    log_info("Order {} invalid input, advancing to InputError", static_cast<unsigned long long>(order_entity.id));
