...
bool is_incoming_arrived_complete(
    const Order& order, float time_in_state,
    const std::map<ItemType, int>& required_counts,
    const std::map<ItemType, int>& received_counts) {
    // Input state - never auto-complete
    return false;
}

bool is_opened_active_complete(const Order& order, float time_in_state,
                               const std::map<ItemType, int>& required_counts,
                               const std::map<ItemType, int>& received_counts) {
    // Input state - never auto-complete
    return false;
...
bool is_requesting_input_error_complete(
    const Order& order, float time_in_state,
    const std::map<ItemType, int>& required_counts,
    const std::map<ItemType, int>& received_counts) {
...
bool is_requesting_all_requested_complete(
    const Order& order, float time_in_state,
    const std::map<ItemType, int>& required_counts,
    const std::map<ItemType, int>& received_counts) {
...
bool is_receiving_on_conveyor_waiting_complete(
    const Order& order, float time_in_state,
    const std::map<ItemType, int>& required_counts,
    const std::map<ItemType, int>& received_counts) {
    // Processing state - complete when conveyor item starts moving
    // TODO: Implement proper conveyor item checking
    // Consider: Query ConveyorItem entities for this order and check if any are
    // moving Currently assumes immediate completion for demo purposes
    return time_in_state >= 0.0f;
}

bool is_receiving_on_conveyor_moving_complete(
    const Order& order, float time_in_state,
    const std::map<ItemType, int>& required_counts,
    const std::map<ItemType, int>& received_counts) {
    // Processing state - complete when item reaches ready area
    // TODO: Implement actual conveyor position checking - currently assumes
    // immediate completion Consider: Query conveyor items for this order and
    // check if any have reached the ready threshold
    return true;  // TODO: implement actual conveyor position checking
}

bool is_receiving_received_to_ready_complete(
    const Order& order, float time_in_state,
    const std::map<ItemType, int>& required_counts,
    const std::map<ItemType, int>& received_counts) {
    ...
    for (const auto& [item_type, required_count] : required_counts) {
        ...
        int received_count =
            (received_it != received_counts.end()) ? received_it->second : 0;
...
bool is_ready_to_box_staged_complete(
    const Order& order, float time_in_state,
    const std::map<ItemType, int>& required_counts,
    const std::map<ItemType, int>& received_counts) {
...
bool is_complete_closeout_delay_complete(
    const Order& order, float time_in_state,
    const std::map<ItemType, int>& required_counts,
    const std::map<ItemType, int>& received_counts) {
    // Processing state - complete after animation finishes (handled by
    // animation system) For now, keep the time-based completion as fallback
...
const std::map<OrderState, StateSpec> STATE_TABLE = {
    // Incoming
    {OrderState::Incoming_Arrived,
     StateSpec{.macro = OrderMacroState::Incoming,
               .kind = OrderStateKind::Input,
               .next = OrderState::Opened_Active,
               .min_duration_seconds = 0.0f,
               .is_complete = is_incoming_arrived_complete,
               .timeline_segment =
                   TimelineSegment{.from_macro = OrderMacroState::Incoming,
                                   .to_macro = OrderMacroState::Opened,
                                   .dash_index = 0,
                                   .dash_count = 1}}},

    // Opened
    {OrderState::Opened_Active,
     StateSpec{.macro = OrderMacroState::Opened,
               .kind = OrderStateKind::Input,
               .next = OrderState::Requesting_NeedsInput,
               .min_duration_seconds = 0.0f,
               .is_complete = is_opened_active_complete,
               .timeline_segment =
                   TimelineSegment{.from_macro = OrderMacroState::Opened,
    ...
    {OrderState::Requesting_NeedsInput,
     StateSpec{
         ...
         .kind = OrderStateKind::Input,
         .next = OrderState::Requesting_AllRequested,  // Will be overridden by
                                                       // input logic
         .min_duration_seconds = 0.0f,
         .is_complete = nullptr,  // Input states don't auto-complete
         .timeline_segment =
             TimelineSegment{.from_macro = OrderMacroState::RequestingItems,
                             .to_macro = OrderMacroState::ReceivingItems,
                             .dash_index = 0,
                             .dash_count = 1}}},
    {OrderState::Requesting_InputError,
     StateSpec{
         .macro = OrderMacroState::RequestingItems,
    ...
    {OrderState::Receiving_OnConveyorWaiting,
     StateSpec{
         ...
         .timeline_segment =
             TimelineSegment{.from_macro = OrderMacroState::ReceivingItems,
    ...
    {OrderState::Receiving_ReceivedToReady,
     StateSpec{
         ...
         .kind = OrderStateKind::Processing,
         .next =
             OrderState::Receiving_OnConveyorWaiting,  // Will be overridden by
                                                       // completion logic
         .min_duration_seconds = 0.0f,
    ...
    {OrderState::ReadyToBox_Staged,
     StateSpec{.macro = OrderMacroState::ReadyToBox,
               .kind = OrderStateKind::Processing,
               .next = OrderState::Boxing_FoldBox,
               .min_duration_seconds = 0.0f,
               .is_complete = is_ready_to_box_staged_complete,
               .timeline_segment =
                   TimelineSegment{.from_macro = OrderMacroState::ReadyToBox,
    ...
    {OrderState::Boxing_FoldBox,
     StateSpec{.macro = OrderMacroState::Boxing,
               .kind = OrderStateKind::Input,
               .next = OrderState::Boxing_PutItems,
               .min_duration_seconds = 0.0f,
               .is_complete = nullptr,
               .timeline_segment =
                   TimelineSegment{.from_macro = OrderMacroState::Boxing,
    ...
    {OrderState::Shipped_Stamp0,
     StateSpec{.macro = OrderMacroState::Shipped,
               .kind = OrderStateKind::Input,
               .next = OrderState::Shipped_Stamp1,
               .min_duration_seconds = 0.0f,
               .is_complete = nullptr,
               .timeline_segment =
                   TimelineSegment{.from_macro = OrderMacroState::Shipped,
    ...
    {OrderState::Complete_ClosedOut,
     StateSpec{.macro = OrderMacroState::Complete,
               ...
               .next = OrderState::Complete_ClosedOut,  // Terminal states don't
                                                        // transition
               .min_duration_seconds = 0.0f,
               .is_complete = nullptr,
               .timeline_segment =
...
OrderMacroState macro_state_of(OrderState state) {
    ...
    return (it != STATE_TABLE.end()) ? it->second.macro
                                     : OrderMacroState::Incoming;
}

OrderStateKind kind_of(OrderState state) {
    auto it = STATE_TABLE.find(state);
    return (it != STATE_TABLE.end()) ? it->second.kind
                                     : OrderStateKind::Terminal;
}
...
TimelineSegment timeline_segment_of(OrderState state) {
    auto it = STATE_TABLE.find(state);
    return (it != STATE_TABLE.end()) ? it->second.timeline_segment
                                     : TimelineSegment{};
}

// State advancement implementation
OrderState advance_if_complete(OrderState current, float dt,
                               float& time_in_state, const Order& order,
                               const std::map<ItemType, int>& required_counts,
                               const std::map<ItemType, int>& received_counts) {
    ...
    if (spec.is_complete &&
        spec.is_complete(order, time_in_state, required_counts,
                         received_counts)) {
        ...
        // For states with conditional next (like Receiving_ReceivedToReady), we
        // need special handling
        if (current == OrderState::Receiving_ReceivedToReady) {
            // Check if all items received - if yes, go to ReadyToBox, else loop
            // back
            bool all_received = true;
            for (const auto& [item_type, required_count] : required_counts) {
                auto received_it = received_counts.find(item_type);
                int received_count = (received_it != received_counts.end())
                                         ? received_it->second
                                         : 0;
                if (received_count < required_count) {
                    all_received = false;
            ...
            time_in_state = 0.0f;
            return all_received ? OrderState::ReadyToBox_Staged
                                : OrderState::Receiving_OnConveyorWaiting;
        }
...
bool try_advance_on_input(Order& order, OrderState& current_state,
                          float& time_in_state,
                          const std::string& input_event) {
